...
## Suggested “video demo script” (5–7 minutes)

1) Show `.env` key settings:
   - `SHOP_ADMIN_EMAIL`, `LOW_STOCK_THRESHOLD`, `DAILY_SALES_REPORT_TIME`, `MAIL_MAILER`
2) Run terminals:
   - `php artisan serve`
   - `npm run dev`
   - `php artisan queue:listen`
   - `php artisan schedule:work` (optional for the report demo)
3) Register a new user at `/register` (or login as `test@example.com`)
4) Browse `/products`, add items to cart
5) Show `/cart`, update qty/remove an item
6) Checkout:
   - Show stock changes on returning to `/products`
   - Show low stock email in logs/inbox
7) Adjust `DAILY_SALES_REPORT_TIME` to a near time, wait, show daily report email
...
git init
git add .
git commit -m "Laravel shopping cart"
git branch -M main
...
2) Add collaborator:
GitHub Repo → Settings → Collaborators → add `@dylanmichaelryan`
